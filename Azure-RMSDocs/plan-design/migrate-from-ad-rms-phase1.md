...
Utilize as seguintes informações para a Fase 1 da migração do AD RMS para o Azure Rights Management (Azure RMS). Estes procedimentos abrangem os passos 1 a 4 de [Migrating from AD RMS to Azure Rights Management (Migrar do AD RMS para o Azure Rights Management – em inglês)](migrate-from-ad-rms-to-azure-rms.md).
...
2.  A partir da consola de gestão do AD RMS (**Serviços de Gestão de Direitos do Active Directory**), expanda o nome do cluster do AD RMS, expanda as **Políticas de Fidedignidade** e, em seguida, clique em **Domínios de Publicação Fidedignos**.
...
-   Proteção HSM através da utilização de um módulo de hardware de segurança da Thales (HSM).

-   Proteção HSM através da utilização de um módulo de hardware de segurança (HSM) a partir de um fornecedor diferente da Thales.

-   Proteção por palavra-passe através de um fornecedor de serviços de criptografia externo.

> [!NOTE]
> Para mais informações sobre a utilização de módulos de hardware de segurança com o AD RMS, consulte [Utilizar o AD RMS com Módulos de Hardware de Segurança](http://technet.microsoft.com/library/jj651024.aspx).
...
Além disso, se os seus modelos no AD RMS utilizaram o grupo **ANYONE**, este grupo será removido automaticamente quando importar os modelos para o Azure RMS, pelo que tem de adicionar manualmente o grupo ou os utilizadores equivalentes, bem como adicionar os mesmos direitos aos modelos importados. O grupo equivalente para o Azure RMS é designado por **AllStaff-7184AB3F-CCD1-46F3-8233-3E09E9CF0E66@ <nome_do_inquilino>.onmicrosoft**. Por exemplo, para a Contoso, este grupo poderá ser algo semelhante a **AllStaff-7184AB3F-CCD1-46F3-8233-3E09E9CF0E66@contoso.onmicrosoft.com**.

Se não tiver a certeza se os seus modelos do AD RMS incluem o grupo ANYONE, pode utilizar o script de exemplo do Windows PowerShell para identificar estes modelos. Para mais informações sobre como utilizar o Windows PowerShell com o AD RMS, consulte [Using Windows PowerShell to Administer AD RMS (Utilizar o Windows PowerShell para Administrar o AD RMS – em inglês)](https://technet.microsoft.com/library/ee221079%28v=ws.10%29.aspx).

Pode ver o grupo automaticamente criado da sua organização se copiar um dos modelos predefinidos da política de direitos no portal clássico do Azure e, em seguida, identificar o **NOME DE UTILIZADOR** na página **DIREITOS**. No entanto, não pode utilizar o portal clássico para adicionar este grupo a um modelo criado ou importado manualmente. Em vez disso, tem de utilizar uma das seguintes opções do PowerShell do Azure RMS:

-   Utilize o cmdlet PowerShell [New-AadrmRightsDefinition](https://msdn.microsoft.com/library/azure/dn727080.aspx) para definir os direitos e o grupo "AllStaff" como um objeto de definição de direitos e execute novamente este comando para cada um dos outros grupos ou utilizadores aos quais já foram concedidos direitos no modelo original juntamente com o grupo ANYONE. Em seguida, adicione os objetos de definição de direitos aos modelos através do cmdlet [Set-AadrmTemplateProperty](https://msdn.microsoft.com/en-us/library/azure/dn727076.aspx).
...
Os modelos que importa do AD RMS assemelham-se e comportam-se tal como os modelos personalizados que pode criar no portal clássico do Azure. Para alterar modelos importados a publicar para que os utilizadores os possam ver e selecionar a partir de aplicações ou para fazer outras alterações aos modelos, consulte [Configuring custom templates for Azure Rights Management (Configurar modelos personalizados para o Azure Rights Management – em inglês)](../deploy-use/configure-custom-templates.md).

> [!TIP]
> Para que os utilizadores tenham uma experiência mais consistente durante o processo de migração, não altere os modelos importados, com exceção destas duas alterações e não publique os dois modelos predefinidos que vêm com o Azure RMS nem crie novos modelos nessa altura. Em vez disso, aguarde até que o processo de migração seja concluído e até ter encerrado os servidores do AD RMS.
