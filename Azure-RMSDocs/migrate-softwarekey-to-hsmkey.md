...
1.  Administrador do Cofre de chaves do Azure: Cada chave SLC exportada que pretende armazenar no Azure Key Vault, siga os passos seguintes no [implementar traga a sua própria chave (BYOK) para o Azure Key Vault](/azure/key-vault/key-vault-hsm-protected-keys#implementing-bring-your-own-key-byok-for-azure-key-vault) seção da documentação do Azure Key Vault:

    -   **Gerar e transferir a chave para o HSM do Azure Key Vault**: [Passo 1: Preparar a estação de trabalho ligada à Internet](/azure/key-vault/key-vault-hsm-protected-keys#step-1-prepare-your-internet-connected-workstation)
...
2. Administrador do Azure Information Protection ou o administrador do Azure Key Vault: Na estação de trabalho desligada, execute a ferramenta TpdUtil a partir da [toolkit de migração do Azure RMS](https://go.microsoft.com/fwlink/?LinkId=524619). Por exemplo, se a ferramenta for instalada na unidade E para a qual copia o ficheiro de dados de configuração com o nome ContosoTPD.xml:
...
> [!IMPORTANT]
> Quando tiver concluído este passo, apague em segurança estes ficheiros PEM da estação de trabalho desligada para assegurar que não podem ser acedidos por pessoas não autorizadas. Por exemplo, execute "cipher /w: E"para eliminar em segurança todos os ficheiros da unidade.
...
Administrador do Cofre de chaves do Azure: Para cada chave SLC exportada que pretende armazenar no Azure Key vault, utilize os passos seguintes a [implementar traga a sua própria chave (BYOK) para o Azure Key Vault](/azure/key-vault/key-vault-hsm-protected-keys#implementing-bring-your-own-key-byok-for-azure-key-vault) seção da documentação do Azure Key Vault:

- [Passo 4: Preparar a transferência da chave](/azure/key-vault/key-vault-hsm-protected-keys#step-4-prepare-your-key-for-transfer)

- [Passo 5: Transferir a chave para o Azure Key Vault](/azure/key-vault/key-vault-hsm-protected-keys#step-5-transfer-your-key-to-azure-key-vault)
...
Antes de transferir a chave para o Azure Key Vault, certifique-se de que o utilitário de KeyTransferRemote.exe retorna **resultado: ÊXITO** quando cria uma cópia da sua chave com permissões reduzidas (passo 4.1) e ao encriptar a chave (passo 4.3).
...
1. Administrador do Azure Information Protection: Na estação de trabalho ligada à Internet e na sessão do PowerShell, copie os novos dados ficheiros de configuração (. xml) que tenham a chave SLC removida depois de executar a ferramenta TpdUtil.
