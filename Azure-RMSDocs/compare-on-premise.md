...
- **É necessária nenhuma infraestrutura servidor**: O Azure Information Protection não necessita dos servidores adicionais e certificados PKI que o AD RMS necessita, porque o Microsoft Azure trata-se um deles para. Isto torna a solução na cloud mais rápida de implementar e mais fácil de manter.
...
- **Suporte incorporado para dispositivos móveis**: Sem alterações de implementação são necessários para o Azure RMS suportar dispositivos móveis e computadores Mac. Para suportar estes dispositivos com o AD RMS, tem de instalar a extensão de dispositivo móvel, configurar o AD FS para federação e criar registos adicionais para o seu serviço DNS público.

- **Modelos predefinidos**: O Azure Information Protection cria modelos predefinidos assim que o serviço de proteção é ativado, o que torna mais fácil começar a proteger dados importantes imediatamente. Não existem modelos predefinidos para o AD RMS.

- **Os modelos departamentais**: Também conhecido como os modelos de âmbito. O Azure Information Protection suporta modelos departamentais para modelos adicionais que criar. Esta configuração permite-lhe especificar um subconjunto de utilizadores para ver modelos específicos nas respetivas aplicações cliente. Limitar o número de modelos que os utilizadores veem torna mais fácil para selecionar a política correta que definir para diferentes grupos de utilizadores. O AD RMS não suporta modelos departamentais.

- **Revogação e controlo de documentos**: O Azure Information Protection suporta estas funcionalidades com o cliente do Azure Information Protection, ao passo que o AD RMS não.

- **Classificação e etiquetagem**: O Azure Information Protection suporta estas funcionalidades com o cliente do Azure Information Protection que se integra com aplicativos do Office e o Explorador de ficheiros, ao passo que o AD RMS não. 


Além disso, uma vez que o Azure Information Protection é um serviço na cloud, pode disponibilizar novas funcionalidades e correções mais rapidamente do que uma solução baseada num servidor no local. Não existem não existem quaisquer funcionalidades novas planeadas para o AD RMS no Windows Server.
...
|Suporte de dispositivos móveis inclui Windows 10 Mobile, Android e iOS.<br /><br />Suporte de e-mail através da IRM do Exchange ActiveSync também é suportado em todas as plataformas de dispositivos móveis que suportem este protocolo.|Suporte de dispositivos móveis inclui Windows 10 Mobile, Android e iOS e requer o [extensão de dispositivo de móveis de serviços do Rights Management Active Directory](https://technet.microsoft.com/library/dn673574.aspx).<br /><br />O suporte de e-mail através da IRM do Exchange ActiveSync é suportado em todas as plataformas de dispositivos móveis que suportem este protocolo.|
...
Para obter mais informações sobre os controlos criptográficos, consulte [controlos criptográficos utilizados pelo Azure RMS: Comprimentos de chave e](how-does-it-work.md#cryptographic-controls-used-by-azure-rms-algorithms-and-key-lengths).
