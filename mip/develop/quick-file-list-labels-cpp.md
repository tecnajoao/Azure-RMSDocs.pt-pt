...
- Concluída [início rápido: inicialização de aplicações de cliente (C++)](quick-app-initialization-cpp.md) primeiro, que cria uma solução do Visual Studio de arranque. Este guia de introdução de "Lista de etiquetas de sensibilidade" baseia-se na anterior, para a criação adequada da solução starter.
...
4. Próximo ao final dos `main()` corpo, abaixo da chave de fechamento `}` da `catch` bloco e acima `return 0;` (onde parou no início rápido anterior), insira o seguinte código:
...
Utilize o seguinte script do PowerShell para gerar tokens de acesso, conforme solicitado pelo SDK. O script utiliza o `Get-ADALToken` cmdlet do módulo ADAL.PS instalado anteriormente, em "MIP SDK instalação e configuração". 
...
2. Guarde o ficheiro de script para que pode executá-lo mais tarde, conforme solicitado pelo seu aplicativo de cliente.
...
3. Para fornecer uma resposta à linha de comandos acima, volte atrás para o script do PowerShell e:
   ...
   - Execute o script do PowerShell. O `Get-ADALToken` cmdlet aciona um prompt de autenticação do Azure AD, semelhante ao seguinte exemplo. Especifique a mesma conta fornecida na saída da consola, no passo 2 de #. Após o início de sessão com êxito, o token de acesso será colocado na área de transferência.
