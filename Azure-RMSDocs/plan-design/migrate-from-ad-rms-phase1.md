...
description: "Utilize as seguintes informações para a Fase 1 da migração do AD RMS para o Azure Rights Management (Azure RMS). Estes procedimentos incluem os passos 1 a 4 do tópico Migrar do AD RMS para o Azure Rights Management."
author: cabailey
manager: mbaldwin
ms.date: 08/17/2016
ms.topic: article
...
Aceda ao Centro de Transferências da Microsoft e transfira a [Ferramenta de Administração do Azure Rights Management](https://go.microsoft.com/fwlink/?LinkId=257721), que contém o módulo de administração do Azure RMS para o Windows PowerShell.
...
> [!NOTE]
> Caso já tenha transferido este módulo do Windows PowerShell, execute o seguinte comando para verificar se o seu número de versão é 2.5.0.0 ou posterior: `(Get-Module aadrm -ListAvailable).Version`
...
> [!IMPORTANT]
> Antes de efetuar este procedimento, confirme primeiro se os servidores do AD RMS estão em execução no Modo Criptográfico 2, que é um requisito para o Azure RMS.
> 
...
Após exportar todos os domínios de publicação fidedignos, está pronto para iniciar a importação destes dados para o Azure RMS.
...
As duas opções de topologia de chaves de inquilino do Azure RMS estão relacionadas com o facto de a sua chave de inquilino poder ser gerida pela Microsoft (**gerida pela Microsoft**) ou por si (**gerida pelo cliente**) no Cofre de Chaves do Azure. Quando faz a gestão da sua própria chave de inquilino do AD RMS, essa gestão é por vezes referida como "bring your own key" (BYOK – Traga a Sua Própria Chave) e requer um módulo de hardware de segurança (HSM) da Thales. Para mais informações, consulte [Planear e implementar a sua chave de inquilino do Azure Rights Management](plan-implement-tenant-key.md).

> [!IMPORTANT]
> O Exchange Online não é atualmente compatível com o BYOK no Azure RMS.  Se quer utilizar o BYOK após a migração e tenciona utilizar o Exchange Online, certifique-se de que compreende como esta configuração reduz a funcionalidade IRM no Exchange Online. Reveja as informações em [BYOK pricing and restrictions (Preços e restrições do BYOK – em inglês)](byok-price-restrictions.md) para o ajudar a escolher a melhor topologia de chaves de inquilino do Azure RMS para a sua migração.
...
|Proteção HSM através da utilização de um módulo de hardware de segurança (HSM) nShield da Thales.|Gerida pelo cliente (BYOK)|Consulte o procedimento de migração **Chave protegida por HMS para chave protegida por HMS ** que se encontra após esta tabela.<br /><br />Este procedimento requer o conjunto de ferramentas BYOK do Cofre de Chaves do Azure e três conjuntos de passos para transferir primeiro a chave do HSM no local para os HSMs do Cofre de Chaves do Azure, em seguida, autorizar o Azure RMS a utilizar a sua chave de inquilino e, por último, transferir os dados de configuração para o Azure RMS.|
|Proteção por palavra-passe na base de dados do AD RMS|Gerida pelo cliente (BYOK)|Consulte o procedimento de migração **Chave protegida por software para chave protegida por HMS ** que se encontra após esta tabela.<br /><br />Este procedimento requer o conjunto de ferramentas BYOK do Cofre de Chaves do Azure e quatro conjuntos de passos para, em primeiro lugar, extrair a sua chave de software e importá-la para um HSM no local, transferir a chave do seu HSM no local para os HSMs do Azure RMS, em seguida transferir os dados do Cofre de Chaves para o Azure RMS e, por último, transferir os dados de configuração para o Azure RMS.|
