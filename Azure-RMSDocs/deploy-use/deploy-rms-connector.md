...
O conector RMS suporta os seguintes servidores no local: Exchange Server, SharePoint Server e servidores de ficheiros que executam o Windows Server e utilizam a Infraestrutura de Classificação de Ficheiros para classificar e aplicar políticas a documentos do Office numa pasta. Se pretender proteger todos os tipos de ficheiros ao utilizar a Classificação de Ficheiros, não utilize o conector RMS, mas sim o [cmdlets Proteção RMS](https://msdn.microsoft.com/library/azure/mt433195.aspx).

> [!NOTE]
> Para ver as versões suportadas destes servidores no local, consulte [Servidores no local que suportam o Azure RMS](..\get-started\requirements-servers.md).
