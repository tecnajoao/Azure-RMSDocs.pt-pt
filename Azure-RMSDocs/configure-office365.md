...
## <a name="exchangeonline-irm-configuration"></a>Exchange Online: Configuração de IRM
Para obter informações sobre como o IRM do Exchange Online funciona com o serviço Azure Rights Management, consulte a [Exchange Online e Exchange Server](office-apps-services-support.md#exchange-online-and-exchange-server) secção de [aplicativos como o Office e serviços suportam o Azure Rights Gestão](office-apps-services-support.md).
...
4. Executar o [Get-IRMConfiguration](https://technet.microsoft.com/library/dd776120(v=exchg.160).aspx) comando para ver a configuração do Exchange Online para o serviço de proteção:
...
5. Para testar esse Exchange Online está configurado com êxito, execute o seguinte comando:
    ...
    ```
    Por exemplo: <strong>Test-IRMConfiguration-remetente  adams@contoso.com</strong>
    
    Este comando executa um conjunto de verificações que inclui a verificação da conectividade ao serviço, a obtenção da configuração, a obtenção de URIs, licenças e modelos. Na sessão do Windows PowerShell, verá os resultados de cada e no final, se nenhuma dessas verificações: **RESULTADO GERAL: PASS**
...
## <a name="sharepointonline-and-onedrive-for-business-irm-configuration"></a>SharePoint Online e OneDrive para empresas: Configuração de IRM

Para obter informações sobre como o IRM do SharePoint Online funciona com o serviço Azure Rights Management, consulte [SharePoint Online e SharePoint Server](office-apps-services-support.md#sharepoint-online-and-sharepoint-server) partir a **proteção do Rights Management** secção isso documentação.

Para configurar o SharePoint Online e OneDrive para empresas para suportar o serviço Azure Rights Management, primeiro tem de ativar o serviço de gestão (IRM) direitos de informação para o SharePoint Online através do Centro de administração do SharePoint. Assim, os proprietários de sites podem proteger as respetivas listas e bibliotecas de documentos do SharePoint com IRM e, além disso, os utilizadores podem proteger a respetiva biblioteca do OneDrive para Empresas com a IRM, para que os documentos aí guardados e partilhados com outras pessoas sejam automaticamente protegidos pelo serviço Azure Rights Management.
...
Para ativar o serviço de gestão (IRM) direitos de informação para o SharePoint Online, consulte as seguintes instruções na documentação do Office:

- [Configurar a Gestão de Direitos de Informação (IRM) no centro de administração do SharePoint](/office365/securitycompliance/set-up-irm-in-sp-admin-center)
...
3. Copie o conteúdo do seguinte script e, no seu computador, dê ao ficheiro o nome Set-IRMOnOneDriveForBusiness.ps1.
   ...
   *&#42;&#42;Disclaimer&#42;&#42;*: Este script de exemplo não é suportado por nenhum serviço ou programa de suporte padrão da Microsoft. Este script de exemplo é fornecido TAL COMO ESTÁ, sem qualquer tipo de garantia.
...
4. Reveja o script e efetue as seguintes alterações:
   ...
   1. Procure o `$sharepointAdminCenterUrl` e substitua o valor de exemplo pelo seu próprio URL do centro de administração do SharePoint.
      ...
      Pode encontrar este valor como o URL de base ao aceder ao centro de administração do SharePoint, com o seguinte formato: https://<em>&lt;nome_de_inquilino&gt;</em>-admin.sharepoint.com
   ...
   3. Procure os `$webUrls` e substitua os valores de exemplo pelos URLs da Web do OneDrive para Empresas dos seus utilizadores, adicionando ou eliminando tantas entradas quantas for necessário.
      ...
      O URL da Web do OneDrive para Empresas do utilizador tem o seguinte formato: https://<em>&lt;nome do inquilino&gt;</em>-my.sharepoint.com/personal/*&lt;nome_de_utilizador&gt;*_*&lt;nome do inquilino&gt;*_com
...
5. Guarde o script e assine-o. Se não assinar o script (mais seguro), tem de configurar o Windows PowerShell no seu computador para executar scripts não assinados. Para tal, execute uma sessão do Windows PowerShell com o **executar como administrador** opção e escreva: **Set-ExecutionPolicy irrestrito**. No entanto, esta configuração permite executar todos os scripts não assinados (menos seguro).
...
Este script também requer o [SDK de Componentes de Cliente do SharePoint Online](https://www.microsoft.com/en-us/download/details.aspx?id=42038) e a [Shell de Gestão do SharePoint Online](https://www.microsoft.com/en-us/download/details.aspx?id=35588). Siga as mesmas instruções para o copiar e colar, guarde o ficheiro localmente (por exemplo, "Report-OneDriveForBusinessSiteInfo.ps1"), modifique os valores `$sharepointAdminCenterUrl` e `$tenantAdmin` como fez anteriormente e, em seguida, execute o script.
